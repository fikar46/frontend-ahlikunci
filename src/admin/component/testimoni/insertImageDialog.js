...
export const InsertImageDialog = props => {
  ...
  const [thumbnailName, setThumbnailName] = React.useState("Upload thumbnail blog")
  ...
  const onAddFiles = event => {
    ...
    // form-data; name="bukti1"; filename="thumbnail_blog_1644066557760.png"
    // form-data; name="files"; filename="coins.png"
    ...
    var file = document.getElementById('gambar_testi');
    ...
    .then((res) => {
        ...
        setUrlImage('https://storage.siapptn.com/image/blog/'+res.data.bukti1)
    }).catch((err)=>{
        console.log(err)
    })
          ...
          <TabStrip selected={selected} onSelect={onTabSelect} animation={false}>
            {Object.entries(attrs).length === 0 && <TabStripTab title="Upload">
              <div className="k-edit-form-container pt-3 pb-3">
                ...
                <div className="k-edit-field">
                  {/* <Input batch={false} multiple={true} defaultFiles={[]} withCredentials={false} saveUrl={'https://demos.telerik.com/kendo-ui/service-v4/upload/save'} removeUrl={'https://demos.telerik.com/kendo-ui/service-v4/upload/remove'} onAdd={onAddFiles} /> */}
                  <Input type="file" id="gambar_testi" name="customFile" label={thumbnailName} onChange={onAddFiles} />
                </div>
                {fields}
                {buttons}
              </div>
